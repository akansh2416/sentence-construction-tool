...
// Function to transform the new JSON structure to match our app's expected format
const transformQuestions = (apiData: any): Question[] => {
  if (!apiData?.data?.questions) {
    return [];
  }

  return apiData.data.questions.map((q: any, index: number) => {
    // Find all blank positions in the question
    const blankPositions: number[] = [];
    let position = 0;
    
    // Split the question at the blank markers
    const parts = q.question.split('_____________');
    const totalBlanks = parts.length - 1;
    
    // Create word options with IDs
    const options = q.options.map((text: string, idx: number) => ({
      id: index * 100 + idx + 1, // Generate unique IDs
      text
    }));
    
    // Create answer words with IDs - matching the correct options
    const answers = q.correctAnswer.map((text: string, idx: number) => {
      const optionIndex = q.options.findIndex((opt: string) => opt === text);
      return {
        id: index * 100 + optionIndex + 1, // Match the ID from options
        text
      };
    });
    
    // Generate blank indices (0, 1, 2, etc.)
    const blanks = Array.from({ length: totalBlanks }, (_, i) => i);

    // Construct sentence with [blank1], [blank2], etc.
    let sentence = '';
    parts.forEach((part, idx) => {
      sentence += part;
      if (idx < parts.length - 1) {
        sentence += `[blank${idx+1}]`;
      }
    });

    return {
      id: index + 1, // Simple numeric ID for internal use
      originalId: q.questionId, // Store the original ID if needed
      sentence: sentence,
      options,
      blanks,
      answers
...
// We're using local data instead of fetching from an API endpoint
export const fetchQuestions = async (): Promise<Question[]> => {
  try {
    // Transform the questions to match our app's format
    return transformQuestions(questionsData);
  } catch (error) {
